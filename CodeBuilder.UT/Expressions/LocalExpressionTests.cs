...
namespace CodeBuilder.UT.Expressions
{
    ...
    {
        private readonly LocalVariable _local = Expr.DeclareLocalVar(typeof(int), "x");
        ...
        {
            var ex = Assert.Throws<ArgumentException>(() => Expr.WriteLocal(Expr.DeclareLocalVar(typeof(object), "o"), Expr.Constant(1)));
        ...
        public void Should_write_and_read_local_variable()
        {
            var func = CreateFunc<int, int>(
                Expr.WriteLocal(_local, Expr.Parameter(0, typeof(int))),
        ...
        {
            var l1 = Expr.DeclareLocalVar(typeof(int), "i1");
            var l2 = Expr.DeclareLocalVar(typeof(object), "i2");
            var l3 = Expr.DeclareLocalVar(typeof(int), "i3");
            var l4 = Expr.DeclareLocalVar(typeof(long), "i4");
            var l5 = Expr.DeclareLocalVar(typeof(short), "i5");

            var func = CreateFunc<int, short>(
                Expr.WriteLocal(l1, Expr.Parameter(0, typeof(int))),
                Expr.WriteLocal(l2, Expr.Convert(Expr.ReadLocal(l1), typeof(object))),
                Expr.WriteLocal(l3, Expr.Convert(Expr.ReadLocal(l2), typeof(int))),
                Expr.WriteLocal(l4, Expr.Convert(Expr.ReadLocal(l3), typeof(long))),
                Expr.WriteLocal(l5, Expr.Convert(Expr.ReadLocal(l4), typeof(short))),
